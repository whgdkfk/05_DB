...
/* 자료형
    ...
    - CHAR(크기) : 고정길이 문자형 (2000 BYTE) 
        ...
        - 속도적인 측면에서 좋음
        - 주민등록번호, 전화번호, 플래그(Y/N)에서 사용, 나머지 VARCHAR2 사용
        
      cf) JAVA CHAR: 문자 1개 저장
          ORACLE CHAR: 고정 길이 문자열 
 ...
 * 이름:     MEMBER_NAME  VARCHAR2(15) (한글 다섯글자까지)
 * 주민번호: MEMBER_SSN   CHAR(14)     (- 포함)
...
INSERT INTO "MEMBER"
VALUES('MEM03', '1Q2W3E4R', '박철수', '971002-1234567', DEFAULT);
 ...
 * 
 * 1) 컬럼 레벨 설정: 테이블 컬럼을 선언할 때 제약 조건 추가
...
/* NOT NULL 제약 조건은 "컬럼 레벨"로만 설정이 가능하다. */
CREATE TABLE USER_USED_NN(
 -- USER_NO NUMBER NOT NULL, -- 제약 조건명 미지정 시 "SYS~" 형태로 자동 지정
...
DROP TABLE USER_USED_UK; -- 컬럼 레벨로 설정하고 테이블 삭제 후 테이블 레벨로 설정
...
CREATE TABLE USER_USED_UK(
    ...
    -- 테이블 레벨로 USER_ID 컬럼에 UNIQUE 제약 조건 지정
...
INSERT INTO USER_USED_UK
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH24_JAR.USER_ID_U)에 위배됩니다.
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH24_JAR.USER_ID_NAME_U)에 위배됩니다.
...
-- NOT NULL + UNIQUE 제약 조건 특징이 합쳐져 있음
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH24_JAR.USER_NO_PK)에 위배됩니다.
-- 기본키 중복으로 오류

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH24_JAR"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
-- 기본키가 NULL이므로 오류
...
CREATE TABLE USER_USED_PK2(
    ...
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    CONSTRAINT PK_USERNO_USERID PRIMARY KEY(USER_NO, USER_ID) -- 복합키(모든 컬럼의 값이 같으면 중복, 한 컬럼이라도 값이 달라야 함)
);
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH24_JAR.PK_USERNO_USERID)에 위배됩니다.
...
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH24_JAR"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다.


----------------------------------------------------------------------------------------------------------------


-- 4. FOREIGN KEY(외부키 / 외래키) 제약 조건
-- FOREIGN KEY가 설정된 컬럼의 값은
-- 외부 테이블의 값을 저장할 수 있다. 
-- ( == 다른 테이블에서 사용할 수 있다.(참조))

-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
-- FOREIGN KEY 제약 조건에 의해서 테이블간의 관계(RELATIONSHIP)가 형성됨
...
-- SQL Error [2291] [23000]: ORA-02291: 무결성 제약조건(KH24_JAR.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다.
...
WHERE GRADE_CODE = 10;
-- SQL Error [2292] [23000]: ORA-02292: 무결성 제약조건(KH24_JAR.GRADE_CODE_FK1)이 위배되었습니다
-- - 자식 레코드가 발견되었습니다 (홍길동, 이순신)
...
CREATE TABLE USER_USED_FK3(
  ...
  -- 테이블 레벨
  -- CONSTRAINT GRADE_CODE_FK3 -- 제약 조건명 설정(생략 가능)
  FOREIGN KEY(GRADE_CODE)
...
-- SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH24_JAR.GENDER_CHECK)이 위배되었습니다.
-- GENDER 컬럼에 CHECK 제약조건으로 '남' 또는 '여'만 기록 가능한데 '남자'라는 조건 이외의 값이 들어와 에러 발생

/*
 * CHECK 제약 조건은 설정이 간편하지만
 * 조건을 수정하는데 어려움이 있다.
 * → 그래서 조건이 다양한 경우에는 
 *   FK 제약 조건을 이용하는 것이 좋다.
 */
...
-- SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH24_JAR.CHECK_SCORE)이 위배되었습니다.
...
DROP TABLE USER_TEST; -- 잘못 만들었을 경우 한 번씩 실행
