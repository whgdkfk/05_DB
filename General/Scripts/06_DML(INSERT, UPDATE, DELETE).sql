...
SELECT * FROM DEPARTMENT2;
ROLLBACK; -- 롤백
SELECT * FROM DEPARTMENT2; -- 롤백 후 원상 복구 확인
...
WHERE
	EMP_NAME IN ('노옹철', '전형돈', '정중하', '하동운'); -- 4행 수정
...
/* 
 * INTO 빠져 있음
 * 왜? MERGE 선언부에 "INTO EMP_M01"이 이미 작성되어서
 */
...
-- 기준 테이블, 같음을 비교할 컬럼을 지정
-- 컬럼 값이 같을 행이 있을 경우 → UPDATE
